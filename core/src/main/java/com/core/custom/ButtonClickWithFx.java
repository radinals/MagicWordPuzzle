...
public abstract class ButtonClickWithFx extends  SingleClickInputListener{
    ...
    public ButtonClickWithFx(final BtnSfxId fxid) {
        ...
        switch(fxid) {
            case FX1 -> fx = GameAssets.getInstance().assetManager.get("audio/fxpop1.wav", Sound.class);
            case FX2 -> fx = GameAssets.getInstance().assetManager.get("audio/fxpop2.wav", Sound.class);
            default -> fx = null;
        }
